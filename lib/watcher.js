...
var watcher = function(dir, accessToken) {
  ...
  watch.createMonitor(dir, {skipUnreadableDir: true}, function(monitor) {
    monitor.on("created", function(file, stat) {
      ...
      cursor.addOrUpdateFiles(dir, {file: stat.mtime}, function(err){
        if(err) {
          console.warn(err);
        }
      });
    ...
    monitor.on("changed", function(file, current) {
      ...
      pushToUploadQueue(task);
      cursor.addOrUpdateFiles(dir, {file: current.mtime}, function(err){
        if(err) {
          console.warn(err);
        }
      });
