...
var uploadFile = function(dir, filePath, accessToken, baseIdentifier, creationDate, cb) {
  async.waterfall([
    function sendDocument(cb) {
      var anyfetch = new AnyfetchClient(accessToken);
      ...
      anyfetch.postDocument(document, rarity.carry([anyfetch],cb));
    },
    function sendFile(anyfetch, document, cb) {
      ...
      anyfetch.getDocumentById(document.id).postFile(fileConfig, cb);
    },
