...
var incrementialSave = function(cursorDir, file, cb) {
  ...
  if(incrementialSave.files.length === 10) {
    // Generate save diffs
    var diffToSave = {};
    incrementialSave.files.forEach(function(file) {
      diffToSave[file.filePath] = file.modificationDate;
    });
...
var savePendingFiles = function(cursorDir) {
  var cursor =  JSON.parse(fs.readFileSync(getCursorPath(cursorDir)).toString());
  incrementialSave.files.forEach(function(file) {
    cursor[file.filePath] = file.modificationDate;
  });
